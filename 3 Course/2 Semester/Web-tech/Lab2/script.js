...
class Maze {
  ...
  generate() {
    const count = localStorage.getItem('levels')
    localStorage.setItem('levels', Number(count)+1)
...
function onRestart(event) {
  window.location.replace('file:///Users/constantineoganesyants/Documents/GitHub/DSTU/3%20Course/2%20Semester/Web-tech/Lab2/index.html')
}

function onKeyDown(event) {
  let cols = document.getElementById("cols").value;
  let rows = document.getElementById("rows").value;
  if (player.row + 1 === Number(cols) && player.col + 1 === Number(rows)) {
    player.col = 0;
    player.row = 0;
    maze.generate();
  }
  ...
  var timeLeft = 10;
    ...
    function countdown() {
      if (timeLeft === 0){
        ...
        const finalLevels = localStorage.getItem('levels');
        const leaders = localStorage.getItem('leaders');
        localStorage.setItem('leaders', leaders.concat(`${finalName}: ${finalLevels};`))

        window.location.replace('file:///Users/constantineoganesyants/Documents/GitHub/DSTU/3%20Course/2%20Semester/Web-tech/Lab2/endGame.html')
      } else {
      ...
      // if (timeLeft === 0){
      //   const finalName = localStorage.getItem('name');
      //   const finalLevels = localStorage.getItem('level');
      //   const leaders = localStorage.getItem('leaders');
      //   localStorage.setItem([leaders.split(';'), ...`${finalName}: ${finalLevels};`])
